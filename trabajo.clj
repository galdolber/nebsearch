...
;;OPTIONS, SETTINGS, DEFAULTS, PRESETS:
;;
    ...
    (when value
      (cond
        ...
        (count global-matcher) (replace-regexes value
                                                global-matcher)
        :else "error"))))
;;la puedo hacer dentro de un let y seguir el orden de arriba a abajo de js
      ...
      (if (= i length) {:filtered filtered
          ...
          (recur (inc i)
                 ...
                 (if (or (and has-function (fn-or-map word))
                   ...
                   (inc countt)
                   countt))))))
#_(remove #(#{"perro" "gato"} %) ["perro" "casa" "comida" "gato"])
...
(defn init [options];;options es para poner otras opciones entre las que esta :presets. :presets despues se mergea con el resto
  (let [{:keys [threshold] :as options} (-> defaults ;; arranco con defaults
        ...
        {:keys [encode] :as options} (update options
                                              :encode
                                              #(or (global-encoder %) %));;PORQUE NECESITO QUE ME DEVUELVA LA FUNCION ACA EN VEZ DEL ENCODE? NO PUEDO PEDIRLA DESPUES DESDE EL ENCODE CON EL GLOBAL ENCODER?
        options (update options
                        :filter
    ...
    (merge options {:map (build-dupes options);;es flexmap ?
                    ...
                    :ids {}
                    :timer 0})))
;;OJO QUE UPDATE ES LA FUNCION DE CLOJURE Y NO UPDATE-FLEX
(init {:resolution 9 :threshold 9 :presets "memory"})
                   ...
                   :else (loop [vals vectorr
                           ...
                           (cond (= cc (count vectorr)) (conj ret [key rett])
                                 ...
                                 :else (recur (rest vals);;vals
                                              (cond (map? (first vals)) (conj rett (remove-index {:map (first vals) :id id}));;rett
                                                    (= id (first vals)) rett
                                                    :else (conj rett (first vals)))
                                              (inc cc)))))
                 (inc c))))))

(remove-index {:map {:c 2 :a [{:b 2} 2]} :id 2})
...
#_(add {:ids {"@1" "aaa"} :tokenize "forward" :split #"\W+" :filter nil
      :threshold 2 :depth 3 :resolution 2 :map [{}{}{}] :rtl false :id 2}
     "bbb" nil false false)
#_{:context-score 0,
 :words ["bbb"],
 :value nil,
          ...
          (if (not (count query))
            ...
            (let [words (if (fn? tokenize)
              ...
              (if (or (not use-contextual) (= ctx-map ctx))
                ...
                [flex {:result []}]))))))))
;;PARA PROBARLA NECESITO TENER EL INDEX INICIADO Y CON DATOS Y LUEGO LA LLAMO CON EL FLEX YA FORMADO Y LAS OPCIONES EXTRAS
...
;;AGARRAR LOS INPUTS DE LOS TESTS Y SEGUIR EL CODIGO EN JS A VER COMO VAN QUEDANDO LAS VARIABLES EN EL CAMINO PARA USARLAS YO COMO MIS INPUTS
;;PREGUNTAR POR EL !SUPPORT-DOCUMENT EN SEARCH, SI VA O NO?
...
;;TEMA FLEX:
;;CAMBIAR LOS INPUTS
;;CAMBIAR LAS LLAMADAS A LAS FUNCIONES EN OTRAS FUNCIONES
;;CAMBIAR LOS OUTPUT DE LAS FUNCIONES PARA NO PERDER DATOS DE FLEX
;;CAMBIAR LOS TESTS
;;ID NO ES DE FLEX/THIS ES UN INPUT CREO
