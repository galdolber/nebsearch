...
#?(:clj
   (do
     ...
     (defn- bt-merge-impl [btree sorted-entries]
       "Merge sorted entries into existing B-tree efficiently.

        Strategy: Merge-sort style merge of existing tree + new entries.
        Walk both in parallel, merge into single sorted sequence, then rebuild tree.

        Advantages over bulk insert:
        - Existing entries already sorted in tree (no re-sort needed)
        - Stream merge (low memory, don't need all entries in vector)
        - Batch writes (like bulk insert)

        Performance: O(n+k) merge + O(n+k) tree build + batch I/O"
       (if (or (empty? sorted-entries) (nil? (:root-offset btree)))
         ;; Empty existing tree or no new entries - use regular bulk insert
         (bt-bulk-insert btree sorted-entries)
         (let [stor (:storage btree)
               ;; Get existing entries as lazy sequence (already sorted)
               existing-entries (bt-seq btree)

               ;; Merge existing + new entries (lazy, memory-efficient)
               ;; Both are sorted, so we can merge like merge-sort
               merged-entries (loop [existing (seq existing-entries)
                                    new-seq (seq sorted-entries)
                                    result (transient [])]
                               (cond
                                 ;; Both exhausted
                                 (and (nil? existing) (nil? new-seq))
                                 (persistent! result)

                                 ;; Only new entries left
                                 (nil? existing)
                                 (persistent! (reduce conj! result new-seq))

                                 ;; Only existing entries left
                                 (nil? new-seq)
                                 (persistent! (reduce conj! result existing))

                                 ;; Both have entries, compare and merge
                                 :else
                                 (let [e1 (first existing)
                                       e2 (first new-seq)
                                       cmp (compare e1 e2)]
                                   (if (<= cmp 0)
                                     (recur (next existing) new-seq (conj! result e1))
                                     (recur existing (next new-seq) (conj! result e2))))))]

           ;; Build new tree from merged entries (like bulk insert does)
           (bt-bulk-insert (->DurableBTree stor nil) merged-entries))))

     (defn bt-merge [btree entries]
       "Merge entries into existing B-tree efficiently.

        Use cases:
        - Adding batch to existing large index (avoids full extraction)
        - Incremental index updates

        Performance compared to alternatives:
        - bt-bulk-insert: O(n+k) extraction + O((n+k)log(n+k)) sort + batch write
        - bt-insert loop: O(k log n) structure + k individual disk writes
        - bt-merge: O(k log n) structure + single batch write ✓

        Best choice when: k << n (small batch, large existing tree)"
       (bt-merge-impl btree entries))
